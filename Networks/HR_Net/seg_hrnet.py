...
def crop(d, g):
    ...
    d1 = d[
        :,
        :,
        int(math.floor((d_h - g_h) / 2.0)) : int(math.floor((d_h - g_h) / 2.0)) + g_h,
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.last_layer = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(
                720, 64, 4, stride=2, padding=1, output_padding=0, bias=True
            ),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(
    ...
    def init_weights(self, pretrained="", train=False):
        ...
        if train == True:
            if os.path.isfile(pretrained):
                ...
                print(
                    "Warning: HRNET ImageNet pretrained model not found at "
                    + pretrained
