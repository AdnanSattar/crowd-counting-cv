...
def main(args):
    ...
    for epoch in range(args["start_epoch"], args["epochs"]):
        ...
        start = time.time()
        train(train_data, model, criterion, optimizer, epoch, args, device)
        end1 = time.time()

        """inference """
        # Save checkpoint every 10 epochs OR at the end of training (for testing with 1 epoch)
        if epoch % 10 == 0 or epoch == args["epochs"] - 1:
            prec1, visi = validate(test_data, model, args, device)
            ...
            print(
                " * best MAE {mae:.3f} ".format(mae=args["best_pred"]),
                args["save_path"],
                end1 - start,
                end2 - end1,
            )
...
def train(Pre_data, model, criterion, optimizer, epoch, args, device="cuda"):
    losses = AverageMeter()
...
def validate(Pre_data, model, args, device="cuda"):
    print("begin test")
    batch_size = 1
    ...
    for i, (fname, img, fidt_map, kpoint) in enumerate(test_loader):
        ...
        if i % 15 == 0:
            print(
                "{fname} Gt {gt:.2f} Pred {pred}".format(
...
def generate_bounding_boxes(kpoint, fname):
    ...
    Img_data = cv2.imread(
        "/home/dkliang/projects/synchronous/datasets/ShanghaiTech/part_A_final/test_data/images/"
        + fname[0]
    )
