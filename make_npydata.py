...
shanghai_root = "./dataset/ShanghaiTech/ShanghaiTech"
jhu_root = "./dataset/jhu_crowd_v2.0"
qnrf_root = "./dataset/UCF-QNRF_ECCV18"
...
try:
    shanghaiBtrain_path = shanghai_root + "/part_B/train_data/images/"
    shanghaiBtest_path = shanghai_root + "/part_B/test_data/images/"
...
try:
    # Check if processed directories exist, otherwise use original structure
    if os.path.exists(qnrf_root + "/train_data/images/"):
        Qnrf_train_path = qnrf_root + "/train_data/images/"
        Qnrf_test_path = qnrf_root + "/test_data/images/"
    else:
        # Use original structure if processed data doesn't exist yet
        Qnrf_train_path = qnrf_root + "/train/images/"
        Qnrf_test_path = qnrf_root + "/test/images/"
...
try:
    # Check if processed directories exist, otherwise use original structure
    if os.path.exists(jhu_root + "/train/images_2048/"):
        Jhu_train_path = jhu_root + "/train/images_2048/"
        Jhu_val_path = jhu_root + "/val/images_2048/"
        jhu_test_path = jhu_root + "/test/images_2048/"
    else:
        # Use original structure if processed data doesn't exist yet
        Jhu_train_path = jhu_root + "/train/images/"
        Jhu_val_path = jhu_root + "/val/images/"
        jhu_test_path = jhu_root + "/test/images/"
...
try:
    ...
    root = "./dataset/NWPU_localization/images_2048/"
    train_img_list = []
    ...
    print(
        "NWPU dataset not found (optional - skipping). The project works fine without it."
    )
